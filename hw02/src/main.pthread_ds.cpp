...
int main(int argc, char* argv[]) {
    ...
    // MAIN program
    // global dynamic scheduling paras
    Dsargs *dsptr = (Dsargs *)malloc(sizeof(Dsargs));
    dsptr->jobsize = 500*500;
    dsptr->curr_idx = 0;
    dsptr->max_idx = xDIM*yDIM;
    dsptr->mutex_ptr = (pthread_mutex_t *)malloc(sizeof(pthread_mutex_t));
    *dsptr->mutex_ptr = PTHREAD_MUTEX_INITIALIZER;
    // create threads
    for (int i = 0; i < nt; i++){
        // calculate start and end index
        int start_idx = xDIM*yDIM/nt * i;
        int end_idx = xDIM*yDIM/nt * (i+1);
        int blocksize = 500*1000;
        args[i] = (Ptargs){.Z=Z, .map=map, .start_idx=start_idx, .end_idx=end_idx, 
                           .iter=iter, .id=i, .time_arr=time_arr, .dsptr=dsptr};
        if (i==nt-1) args[i].end_idx = xDIM*yDIM;

        // create independent threads
        pthread_create(&threads[i], NULL, mandelbrot_loop_pt_ds, (void *)(&args[i]));
    }
    // join threads
    for (int i = 0; i < nt; i++){
    ...
    // record data
    if (record==1){
        runtime_record("pth_ds", DIM, nt, t, t_sum);
        runtime_record_detail("pth_ds", DIM, nt, t, time_arr);
    }

    // save png
    if (save==1) mandelbrot_save("pth_ds", map, xDIM, yDIM);
